...
def make_response():
    return dict(status="success", data={"resultType": "matrix", "result": []})


def add_result(response, labels, ts_df):
    response["data"]["result"].append(dict(metric=labels, values=to_prom_values(ts_df)))
...
def add_alert_result(response, labels, start, end, alertname):
    labels = labels.copy()
    labels.update(dict(__name__="ALERTS", alertname=alertname, alertstate="firing"))
...
def make():
    ...
    add_alert_result(
        res,
        {"device": "/dev/sdb1", "severity": "critical"},
    ...
    add_alert_result(
        res,
        {"cron": "some-cron-task", "severity": "warning"},
    ...
    add_alert_result(
        res,
        {"cron": "some-other-task", "severity": "warning"},
    ...
    expectations.append(
        {
            "httpRequest": {
                ...
                "queryStringParameters": {"query": ["ALERTS"]},
            },
    ...
    expectations.append(
        {
            "httpRequest": {
                "path": "/api/v1/query_range",
                "queryStringParameters": {"query": [".*node_cpu_seconds_total.*"]},
            },
            "httpResponse": {"body": res},
        }
    )
